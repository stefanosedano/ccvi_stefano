...
def custom_norm(df,count_column):
    ...
    df[f"{count_column}_multiply_pop_density"] = df[count_column] * df["wp_pop_density"]
    ...
    transformed_data, best_lambda = boxcox(df[f"{count_column}_multiply_pop_density_log"])
